...
def show_newxx(newxx):
    sql="select * from `app_newxx`.`"+newxx+"` order by `id` desc limit 10"
    return get_record(sql)

def show_newxx_file(newxx,newxxid):
    sql="select * from `app_newxx`.`"+newxx+"_file` where `fileid`="+newxxid+" order by `id` desc limit 1"
...
def view_newxx_file(newxx,id):
    sql="select * from `app_newxx`.`"+newxx+"_file` where `id`="+id
    row=get_one_record(sql)
    if row: return "<pre><code>"+row[4]+"</code></pre>"
    return 'ko'

def view_newxx_files(newxx):
    sql="select * from `app_newxx`.`"+newxx+"_file` order by `id` desc"
...
def update_record(table,which,who,what,where):
    ...
    connection.select_db(MYSQL_DB)
    try:
        connection.query("INSERT INTO `app_newxx`.`"+table+"` ( `who`, `which`, `what`, `where`) VALUES ( '"+who+"', '"+which+"', '"+what+"', '"+where+"')")
...
def search_newxx(newxx,keywords):
    ...
    sql="select * from `app_newxx`.`"+newxx+"_file` where `content` like '%"+keywords+"%'"
    try:
...
def upload_file(table,newxxid,author,filename,content):
    ...
    connection.select_db(MYSQL_DB)
    try:
        connection.query("INSERT INTO `app_newxx`.`"+table+"` ( `fileid`, `author`, `filename`, `content`) VALUES ( '"+newxxid+"', '"+author+"', '"+filename+"', '"+content+"')")
        return 'ok'
    except NameError, e:
        print e
    except:
